...
class PeoplesComparatorTest extends Unit
{
    public function testRun()
    {
        ...
        $finder = new PeopleComparator($peoples, $findedPeople, $comparator);
        $results = $finder->compare();
        $this->assertCount(2, $results);
        foreach ($results as $result) {
            $this->assertInstanceOf(Result::class, $result);
            if ($result->getQuality() == 100) {
                $this->assertEquals(spl_object_hash($people), spl_object_hash($result->getPerson()));
            } else {
                $this->assertEquals(0, $result->getQuality());
                $this->assertEquals(spl_object_hash($otherPeople), spl_object_hash($result->getPerson()));
            }
        }
    }
}